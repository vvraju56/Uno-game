/**
 * 10-Player UNO Game Test Suite
 * Tests all game mechanics for 10-player games
 */

const { assert } = require('chai');
const UnoGame10Player = require('../../player-count-rules/UnoGame10Player');

describe('10-Player UNO Game Test Suite', function() {
  this.timeout(10000);
  ...
  beforeEach(function() {
    game = new UnoGame10Player();
    room = game.createRoom('TEST-10P');
  });

  describe('Game Setup', function() {
    it('should create room for 10 players', function() {
    ...
    it('should add exactly 10 players', function() {
      for (let i = 1; i <= 10; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
      
      assert.equal(room.players.length, 10);
      assert.throws(() => game.addPlayer(room, { id: `p${11`, name: `Player ${11}` }), 'Room is full');
    });
  ...
  describe('10-Player Skip Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 10; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('10-Player Reverse Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 10; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('10-Player Draw Two Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 10; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('10-Player Wild Draw Four Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 10; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('10-Player Turn Order Logic', function() {
    beforeEach(function() {
      for (let i = 1; i <= 10; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
    ...
    it('should wrap around correctly in clockwise direction', function() {
      ...
      room.currentPlayerIndex = 9; // Last player
    ...
    it('should wrap around correctly in counter-clockwise direction', function() {
      ...
      const nextIndex = game.getNextPlayerIndex(room);
      assert.equal(nextIndex, 9); // Should go to last player
    });
    ...
    it('should calculate relative positions correctly', function() {
      ...
      const positions = [
        ...
        { index: 9, expected: 'previous' } // Last player is previous
      ];
      
      positions.forEach(({ index, expected }) => {
        const relativePos = (index - room.currentPlayerIndex + 10 + 10 * 10) % 10;
        let actual;
        if (relativePos === 0) actual = 'current';
        else if (relativePos === 1) actual = 'next';
        else if (relativePos === 9) actual = 'previous';
        else if (relativePos === Math.floor(10 / 2)) actual = 'opposite';
  ...
  describe('10-Player Edge Cases', function() {
    beforeEach(function() {
      for (let i = 1; i <= 10; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('Rule Descriptions', function() {
    it('should return correct rule descriptions', function() {
      ...
      assert.equal(rules.playerCount, 10);
      assert.include(rules.description, '10-Player');
  ...
  describe('Game State Management', function() {
    it('should return complete game state', function() {
      ...
      assert.equal(gameState.players.length, 10);
      ...
      assert.isTrue(gameState[`10PlayerRules`]);
    });
  });
