/**
 * 6-Player UNO Game Test Suite
 * Tests all game mechanics for 6-player games
 */

const { assert } = require('chai');
const UnoGame6Player = require('../../player-count-rules/UnoGame6Player');

describe('6-Player UNO Game Test Suite', function() {
  this.timeout(10000);
  ...
  beforeEach(function() {
    game = new UnoGame6Player();
    room = game.createRoom('TEST-6P');
  });

  describe('Game Setup', function() {
    it('should create room for 6 players', function() {
    ...
    it('should add exactly 6 players', function() {
      for (let i = 1; i <= 6; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
      
      assert.equal(room.players.length, 6);
      assert.throws(() => game.addPlayer(room, { id: `p${7`, name: `Player ${7}` }), 'Room is full');
    });
  ...
  describe('6-Player Skip Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 6; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('6-Player Reverse Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 6; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('6-Player Draw Two Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 6; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('6-Player Wild Draw Four Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 6; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('6-Player Turn Order Logic', function() {
    beforeEach(function() {
      for (let i = 1; i <= 6; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
    ...
    it('should wrap around correctly in clockwise direction', function() {
      ...
      room.currentPlayerIndex = 5; // Last player
    ...
    it('should wrap around correctly in counter-clockwise direction', function() {
      ...
      const nextIndex = game.getNextPlayerIndex(room);
      assert.equal(nextIndex, 5); // Should go to last player
    });
    ...
    it('should calculate relative positions correctly', function() {
      ...
      const positions = [
        ...
        { index: 5, expected: 'previous' } // Last player is previous
      ];
      
      positions.forEach(({ index, expected }) => {
        const relativePos = (index - room.currentPlayerIndex + 6 + 6 * 6) % 6;
        let actual;
        if (relativePos === 0) actual = 'current';
        else if (relativePos === 1) actual = 'next';
        else if (relativePos === 5) actual = 'previous';
        else if (relativePos === Math.floor(6 / 2)) actual = 'opposite';
  ...
  describe('6-Player Edge Cases', function() {
    beforeEach(function() {
      for (let i = 1; i <= 6; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('Rule Descriptions', function() {
    it('should return correct rule descriptions', function() {
      ...
      assert.equal(rules.playerCount, 6);
      assert.include(rules.description, '6-Player');
  ...
  describe('Game State Management', function() {
    it('should return complete game state', function() {
      ...
      assert.equal(gameState.players.length, 6);
      ...
      assert.isTrue(gameState[`6PlayerRules`]);
    });
  });
