/**
 * 9-Player UNO Game Test Suite
 * Tests all game mechanics for 9-player games
 */

const { assert } = require('chai');
const UnoGame9Player = require('../../player-count-rules/UnoGame9Player');

describe('9-Player UNO Game Test Suite', function() {
  this.timeout(10000);
  ...
  beforeEach(function() {
    game = new UnoGame9Player();
    room = game.createRoom('TEST-9P');
  });

  describe('Game Setup', function() {
    it('should create room for 9 players', function() {
    ...
    it('should add exactly 9 players', function() {
      for (let i = 1; i <= 9; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
      
      assert.equal(room.players.length, 9);
      assert.throws(() => game.addPlayer(room, { id: `p${10`, name: `Player ${10}` }), 'Room is full');
    });
  ...
  describe('9-Player Skip Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 9; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('9-Player Reverse Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 9; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('9-Player Draw Two Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 9; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('9-Player Wild Draw Four Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 9; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('9-Player Turn Order Logic', function() {
    beforeEach(function() {
      for (let i = 1; i <= 9; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
    ...
    it('should wrap around correctly in clockwise direction', function() {
      ...
      room.currentPlayerIndex = 8; // Last player
    ...
    it('should wrap around correctly in counter-clockwise direction', function() {
      ...
      const nextIndex = game.getNextPlayerIndex(room);
      assert.equal(nextIndex, 8); // Should go to last player
    });
    ...
    it('should calculate relative positions correctly', function() {
      ...
      const positions = [
        ...
        { index: 4, expected: 'opposite' }, // Opposite player
        { index: 8, expected: 'previous' } // Last player is previous
      ];
      
      positions.forEach(({ index, expected }) => {
        const relativePos = (index - room.currentPlayerIndex + 9 + 9 * 9) % 9;
        let actual;
        if (relativePos === 0) actual = 'current';
        else if (relativePos === 1) actual = 'next';
        else if (relativePos === 8) actual = 'previous';
        else if (relativePos === Math.floor(9 / 2)) actual = 'opposite';
  ...
  describe('9-Player Edge Cases', function() {
    beforeEach(function() {
      for (let i = 1; i <= 9; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('Rule Descriptions', function() {
    it('should return correct rule descriptions', function() {
      ...
      assert.equal(rules.playerCount, 9);
      assert.include(rules.description, '9-Player');
  ...
  describe('Game State Management', function() {
    it('should return complete game state', function() {
      ...
      assert.equal(gameState.players.length, 9);
      ...
      assert.isTrue(gameState[`9PlayerRules`]);
    });
  });
