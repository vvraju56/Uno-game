/**
 * 5-Player UNO Game Test Suite
 * Tests all game mechanics for 5-player games
 */

const { assert } = require('chai');
const UnoGame5Player = require('../../player-count-rules/UnoGame5Player');

describe('5-Player UNO Game Test Suite', function() {
  this.timeout(10000);
  ...
  beforeEach(function() {
    game = new UnoGame5Player();
    room = game.createRoom('TEST-5P');
  });

  describe('Game Setup', function() {
    it('should create room for 5 players', function() {
    ...
    it('should add exactly 5 players', function() {
      for (let i = 1; i <= 5; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
      
      assert.equal(room.players.length, 5);
      assert.throws(() => game.addPlayer(room, { id: `p${6`, name: `Player ${6}` }), 'Room is full');
    });
  ...
  describe('5-Player Skip Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 5; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('5-Player Reverse Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 5; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('5-Player Draw Two Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 5; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('5-Player Wild Draw Four Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 5; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('5-Player Turn Order Logic', function() {
    beforeEach(function() {
      for (let i = 1; i <= 5; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
    ...
    it('should wrap around correctly in clockwise direction', function() {
      ...
      room.currentPlayerIndex = 4; // Last player
    ...
    it('should wrap around correctly in counter-clockwise direction', function() {
      ...
      const nextIndex = game.getNextPlayerIndex(room);
      assert.equal(nextIndex, 4); // Should go to last player
    });
    ...
    it('should calculate relative positions correctly', function() {
      ...
      const positions = [
        ...
        { index: 2, expected: 'opposite' }, // Opposite player
        { index: 4, expected: 'previous' } // Last player is previous
      ];
      
      positions.forEach(({ index, expected }) => {
        const relativePos = (index - room.currentPlayerIndex + 5 + 5 * 5) % 5;
        let actual;
        if (relativePos === 0) actual = 'current';
        else if (relativePos === 1) actual = 'next';
        else if (relativePos === 4) actual = 'previous';
        else if (relativePos === Math.floor(5 / 2)) actual = 'opposite';
  ...
  describe('5-Player Edge Cases', function() {
    beforeEach(function() {
      for (let i = 1; i <= 5; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('Rule Descriptions', function() {
    it('should return correct rule descriptions', function() {
      ...
      assert.equal(rules.playerCount, 5);
      assert.include(rules.description, '5-Player');
  ...
  describe('Game State Management', function() {
    it('should return complete game state', function() {
      ...
      assert.equal(gameState.players.length, 5);
      ...
      assert.isTrue(gameState[`5PlayerRules`]);
    });
  });
