/**
 * 11-Player UNO Game Test Suite
 * Tests all game mechanics for 11-player games
 */

const { assert } = require('chai');
const UnoGame11Player = require('../../player-count-rules/UnoGame11Player');

describe('11-Player UNO Game Test Suite', function() {
  this.timeout(10000);
  ...
  beforeEach(function() {
    game = new UnoGame11Player();
    room = game.createRoom('TEST-11P');
  });

  describe('Game Setup', function() {
    it('should create room for 11 players', function() {
      assert.equal(room.playerCount, 11);
    ...
    it('should add exactly 11 players', function() {
      for (let i = 1; i <= 11; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
      
      assert.equal(room.players.length, 11);
      assert.throws(() => game.addPlayer(room, { id: `p${12`, name: `Player ${12}` }), 'Room is full');
    });
  ...
  describe('11-Player Skip Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 11; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('11-Player Reverse Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 11; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('11-Player Draw Two Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 11; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('11-Player Wild Draw Four Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 11; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('11-Player Turn Order Logic', function() {
    beforeEach(function() {
      for (let i = 1; i <= 11; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
    ...
    it('should wrap around correctly in clockwise direction', function() {
      ...
      room.currentPlayerIndex = 10; // Last player
    ...
    it('should wrap around correctly in counter-clockwise direction', function() {
      ...
      const nextIndex = game.getNextPlayerIndex(room);
      assert.equal(nextIndex, 10); // Should go to last player
    });
    ...
    it('should calculate relative positions correctly', function() {
      ...
      const positions = [
        ...
        { index: 5, expected: 'opposite' }, // Opposite player
        { index: 10, expected: 'previous' } // Last player is previous
      ];
      
      positions.forEach(({ index, expected }) => {
        const relativePos = (index - room.currentPlayerIndex + 11 + 11 * 11) % 11;
        let actual;
        if (relativePos === 0) actual = 'current';
        else if (relativePos === 1) actual = 'next';
        else if (relativePos === 10) actual = 'previous';
        else if (relativePos === Math.floor(11 / 2)) actual = 'opposite';
  ...
  describe('11-Player Edge Cases', function() {
    beforeEach(function() {
      for (let i = 1; i <= 11; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('Rule Descriptions', function() {
    it('should return correct rule descriptions', function() {
      ...
      assert.equal(rules.playerCount, 11);
      assert.include(rules.description, '11-Player');
  ...
  describe('Game State Management', function() {
    it('should return complete game state', function() {
      ...
      assert.equal(gameState.players.length, 11);
      ...
      assert.isTrue(gameState[`11PlayerRules`]);
    });
  });
