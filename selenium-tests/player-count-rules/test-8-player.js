/**
 * 8-Player UNO Game Test Suite
 * Tests all game mechanics for 8-player games
 */

const { assert } = require('chai');
const UnoGame8Player = require('../../player-count-rules/UnoGame8Player');

describe('8-Player UNO Game Test Suite', function() {
  this.timeout(10000);
  ...
  beforeEach(function() {
    game = new UnoGame8Player();
    room = game.createRoom('TEST-8P');
  });

  describe('Game Setup', function() {
    it('should create room for 8 players', function() {
      assert.equal(room.playerCount, 8);
    ...
    it('should add exactly 8 players', function() {
      for (let i = 1; i <= 8; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
      
      assert.equal(room.players.length, 8);
      assert.throws(() => game.addPlayer(room, { id: `p${9`, name: `Player ${9}` }), 'Room is full');
    });
  ...
  describe('8-Player Skip Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 8; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('8-Player Reverse Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 8; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('8-Player Draw Two Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 8; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('8-Player Wild Draw Four Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 8; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('8-Player Turn Order Logic', function() {
    beforeEach(function() {
      for (let i = 1; i <= 8; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
    ...
    it('should wrap around correctly in clockwise direction', function() {
      ...
      room.currentPlayerIndex = 7; // Last player
    ...
    it('should wrap around correctly in counter-clockwise direction', function() {
      ...
      const nextIndex = game.getNextPlayerIndex(room);
      assert.equal(nextIndex, 7); // Should go to last player
    });
    ...
    it('should calculate relative positions correctly', function() {
      ...
      const positions = [
        ...
        { index: 7, expected: 'previous' } // Last player is previous
      ];
      
      positions.forEach(({ index, expected }) => {
        const relativePos = (index - room.currentPlayerIndex + 8 + 8 * 8) % 8;
        let actual;
        if (relativePos === 0) actual = 'current';
        else if (relativePos === 1) actual = 'next';
        else if (relativePos === 7) actual = 'previous';
        else if (relativePos === Math.floor(8 / 2)) actual = 'opposite';
  ...
  describe('8-Player Edge Cases', function() {
    beforeEach(function() {
      for (let i = 1; i <= 8; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('Rule Descriptions', function() {
    it('should return correct rule descriptions', function() {
      ...
      assert.equal(rules.playerCount, 8);
      assert.include(rules.description, '8-Player');
  ...
  describe('Game State Management', function() {
    it('should return complete game state', function() {
      ...
      assert.equal(gameState.players.length, 8);
      ...
      assert.isTrue(gameState[`8PlayerRules`]);
    });
  });
