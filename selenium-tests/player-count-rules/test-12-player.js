/**
 * 12-Player UNO Game Test Suite
 * Tests all game mechanics for 12-player games
 */

const { assert } = require('chai');
const UnoGame12Player = require('../../player-count-rules/UnoGame12Player');

describe('12-Player UNO Game Test Suite', function() {
  this.timeout(10000);
  ...
  beforeEach(function() {
    game = new UnoGame12Player();
    room = game.createRoom('TEST-12P');
  });

  describe('Game Setup', function() {
    it('should create room for 12 players', function() {
      assert.equal(room.playerCount, 12);
    ...
    it('should add exactly 12 players', function() {
      for (let i = 1; i <= 12; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
      
      assert.equal(room.players.length, 12);
      assert.throws(() => game.addPlayer(room, { id: `p${13`, name: `Player ${13}` }), 'Room is full');
    });
  ...
  describe('12-Player Skip Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 12; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('12-Player Reverse Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 12; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('12-Player Draw Two Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 12; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('12-Player Wild Draw Four Card Rules', function() {
    beforeEach(function() {
      for (let i = 1; i <= 12; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('12-Player Turn Order Logic', function() {
    beforeEach(function() {
      for (let i = 1; i <= 12; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
    ...
    it('should wrap around correctly in clockwise direction', function() {
      ...
      room.currentPlayerIndex = 11; // Last player
    ...
    it('should wrap around correctly in counter-clockwise direction', function() {
      ...
      const nextIndex = game.getNextPlayerIndex(room);
      assert.equal(nextIndex, 11); // Should go to last player
    });
    ...
    it('should calculate relative positions correctly', function() {
      ...
      const positions = [
        ...
        { index: 6, expected: 'opposite' }, // Opposite player
        { index: 11, expected: 'previous' } // Last player is previous
      ];
      
      positions.forEach(({ index, expected }) => {
        const relativePos = (index - room.currentPlayerIndex + 12 + 12 * 12) % 12;
        let actual;
        if (relativePos === 0) actual = 'current';
        else if (relativePos === 1) actual = 'next';
        else if (relativePos === 11) actual = 'previous';
        else if (relativePos === Math.floor(12 / 2)) actual = 'opposite';
  ...
  describe('12-Player Edge Cases', function() {
    beforeEach(function() {
      for (let i = 1; i <= 12; i++) {
        game.addPlayer(room, { id: `p${i}`, name: `Player ${i}` });
      }
  ...
  describe('Rule Descriptions', function() {
    it('should return correct rule descriptions', function() {
      ...
      assert.equal(rules.playerCount, 12);
      assert.include(rules.description, '12-Player');
  ...
  describe('Game State Management', function() {
    it('should return complete game state', function() {
      ...
      assert.equal(gameState.players.length, 12);
      ...
      assert.isTrue(gameState[`12PlayerRules`]);
    });
  });
