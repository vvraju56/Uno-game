/**
 * UNO GAME - 3 PLAYERS COMPLETE IMPLEMENTATION
 * 
...
class UnoGame3Player {
  ...
  /**
   * Create game room for 3 players
   */
  createRoom(roomId) {
  ...
  startGame(room) {
    if (room.players.length !== 3) {
      throw new Error('Need exactly 3 players to start 3-player game');
    }
    ...
    if (firstCard.value === 'skip') {
      ...
      room.skipEffect = {
        ...
        nextPlayerIndex: 2,
        reason: '3-player rule: skip next player'
      };
    } else if (firstCard.value === 'reverse') {
      // Reverse: direction changes, player 2 plays in new direction
      room.direction *= -1;
      room.currentPlayerIndex = 2; // Player 2 in counter-clockwise direction
      room.reverseEffect = {
        direction: room.direction > 0 ? 'clockwise' : 'counter-clockwise',
        playersCount: 3,
        reason: '3-player rule: direction change only'
      };
      ...
      room.drawEffect = { 
        ...
        playerName: room.players[1].name,
        nextPlayerIndex: 2,
        reason: '3-player rule: target draws and loses turn'
      };
  ...
  playCard(room, playerId, cardId, chosenColor = null) {
    ...
    // Apply 3-player specific power card effects
    ...
    switch (card.value) {
      case 'skip':
        ...
        room.skipEffect = {
          ...
          reason: '3-player rule: next player loses turn'
        };
        
        finalNextPlayerIndex = playerAfterSkippedIndex;
        console.log(`${skippedPlayer.name} skipped. ${playerAfterSkipped.name} plays next (3-player Skip rule)`);
        break;
      ...
      case 'reverse':
        ...
        console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${room.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (3-player Reverse rule)`);
        break;
      ...
      case 'draw2':
        ...
        console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${playerAfterTarget.name} plays next (3-player Draw Two rule)`);
        break;
      ...
      case 'wild4':
        ...
        console.log(`${wild4Target.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${playerAfterWild4.name} plays next (3-player Wild Draw Four rule)`);
        break;
        
      case 'wild':
        // Wild: normal turn progression in 3-player
        room.wildEffect = {
          colorChosen: true,
          playersCount: 3,
          chosenColor: chosenColor
        ...
        console.log(`Color changed to ${chosenColor}. ${nextAfterWild.name} plays next (3-player Wild rule)`);
        break;
        
      default:
        // Number card: normal turn progression
        const nextAfterNumber = room.players[nextPlayerIndex];
        console.log(`${player.name} played ${card.color} ${card.value}. ${nextAfterNumber.name} plays next (3-player normal rule)`);
    }
  ...
  getNextPlayerIndexFrom(room, fromPlayerIndex) {
    return (fromPlayerIndex + room.direction + 3) % 3;
  }
  ...
  getNextPlayerIndex(room) {
    return (room.currentPlayerIndex + room.direction + 3) % 3;
  }
  ...
  /**
   * Get 3-player rule description
   */
  getPlayerCountRules() {
    return {
      playerCount: 3,
      description: '3-Player UNO Rules (Standard)',
      rules: {
        skip: 'Next player loses turn, player after them plays',
        reverse: 'Direction changes only, next player in new direction plays',
      ...
      specialBehaviors: [
        ...
        'Turn order wraps around 3 players',
