/**
 * UNO GAME - 7 PLAYERS COMPLETE IMPLEMENTATION
 * 
 * Standard 7-player rules:
...
class UnoGame7Player {
  ...
  /**
   * Create game room for 7 players
   */
  createRoom(roomId) {
  ...
  /**
   * Start game with 7 players
   */
  startGame(room) {
    if (room.players.length !== 7) {
      throw new Error('Need exactly 7 players to start 7-player game');
    }
    ...
    if (firstCard.value === 'skip') {
      ...
      room.skipEffect = {
        ...
        nextPlayerIndex: 2,
        reason: '7-player rule: skip next player'
      };
    } else if (firstCard.value === 'reverse') {
      room.direction *= -1;
      room.currentPlayerIndex = 6; // Last player in counter-clockwise
      room.reverseEffect = {
        direction: room.direction > 0 ? 'clockwise' : 'counter-clockwise',
        playersCount: 7,
        reason: '7-player rule: direction change only'
      };
      ...
      room.drawEffect = { 
        ...
        playerName: room.players[1].name,
        nextPlayerIndex: 2,
        reason: '7-player rule: target draws and loses turn'
      };
  ...
  playCard(room, playerId, cardId, chosenColor = null) {
    ...
    // Apply 7-player specific power card effects
    ...
    switch (card.value) {
      case 'skip':
        ...
        room.skipEffect = {
          ...
          reason: '7-player rule: next player loses turn'
        };
        
        finalNextPlayerIndex = playerAfterSkippedIndex;
        console.log(`${skippedPlayer.name} skipped. ${playerAfterSkipped.name} plays next (7-player Skip rule)`);
        break;
      ...
      case 'reverse':
        ...
        console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${room.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (7-player Reverse rule)`);
        break;
      ...
      case 'draw2':
        ...
        console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${playerAfterTarget.name} plays next (7-player Draw Two rule)`);
        break;
      ...
      case 'wild4':
        ...
        console.log(`${wild4Target.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${playerAfterWild4.name} plays next (7-player Wild Draw Four rule)`);
        break;
        
      case 'wild':
        // Wild: normal turn progression in 7-player
        room.wildEffect = {
          colorChosen: true,
          playersCount: 7,
          chosenColor: chosenColor
        ...
        console.log(`Color changed to ${chosenColor}. ${nextAfterWild.name} plays next (7-player Wild rule)`);
        break;
        
      default:
        // Number card: normal turn progression
        const nextAfterNumber = room.players[nextPlayerIndex];
        console.log(`${player.name} played ${card.color} ${card.value}. ${nextAfterNumber.name} plays next (7-player normal rule)`);
    }
  ...
  getNextPlayerIndexFrom(room, fromPlayerIndex) {
    return (fromPlayerIndex + room.direction + 7) % 7;
  }
  ...
  getNextPlayerIndex(room) {
    return (room.currentPlayerIndex + room.direction + 7) % 7;
  }
  ...
  /**
   * Get 7-player rule description
   */
  getPlayerCountRules() {
    return {
      playerCount: 7,
      description: '7-Player UNO Rules (Standard)',
      rules: {
        skip: 'Next player loses turn, player after them plays',
        reverse: 'Direction changes only, next player in new direction plays',
      ...
      specialBehaviors: [
        ...
        'Turn order wraps around 7 players',
        'Strategic positioning becomes important with 7 players'
      ]
    };
  }
}

module.exports = UnoGame7Player;
