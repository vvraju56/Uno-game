/**
 * UNO GAME - 4 PLAYERS COMPLETE IMPLEMENTATION
 * 
...
class UnoGame4Player {
  ...
  /**
   * Create game room for 4 players
   */
  createRoom(roomId) {
  ...
  startGame(room) {
    if (room.players.length !== 4) {
      throw new Error('Need exactly 4 players to start 4-player game');
    }
    ...
    if (firstCard.value === 'skip') {
      // Skip: player 1 loses turn, player 2 plays
      room.currentPlayerIndex = this.getNextPlayerIndex(room);
      room.skipEffect = {
        playerId: room.players[1].id,
        playerName: room.players[1].name,
        nextPlayerIndex: 2,
        reason: '4-player rule: skip next player'
      };
    } else if (firstCard.value === 'reverse') {
      // Reverse: direction changes, player 3 plays in new direction
      room.direction *= -1;
      room.currentPlayerIndex = 3; // Player 3 in counter-clockwise direction
      room.reverseEffect = {
        direction: room.direction > 0 ? 'clockwise' : 'counter-clockwise',
        playersCount: 4,
        reason: '4-player rule: direction change only'
      };
    } else if (firstCard.value === 'draw2') {
      // Draw Two: player 1 draws 2 cards and loses turn, player 2 plays
      ...
      room.drawEffect = { 
        ...
        playerName: room.players[1].name,
        nextPlayerIndex: 2,
        reason: '4-player rule: target draws and loses turn'
      };
  ...
  playCard(room, playerId, cardId, chosenColor = null) {
    ...
    // Apply 4-player specific power card effects
    ...
    switch (card.value) {
      case 'skip':
        ...
        room.skipEffect = {
          ...
          reason: '4-player rule: next player loses turn'
        };
        
        finalNextPlayerIndex = playerAfterSkippedIndex;
        console.log(`${skippedPlayer.name} skipped. ${playerAfterSkipped.name} plays next (4-player Skip rule)`);
        break;
      ...
      case 'reverse':
        ...
        console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${room.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (4-player Reverse rule)`);
        break;
      ...
      case 'draw2':
        ...
        console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${playerAfterTarget.name} plays next (4-player Draw Two rule)`);
        break;
      ...
      case 'wild4':
        ...
        console.log(`${wild4Target.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${playerAfterWild4.name} plays next (4-player Wild Draw Four rule)`);
        break;
        
      case 'wild':
        // Wild: normal turn progression in 4-player
        room.wildEffect = {
          colorChosen: true,
          playersCount: 4,
          chosenColor: chosenColor
        ...
        console.log(`Color changed to ${chosenColor}. ${nextAfterWild.name} plays next (4-player Wild rule)`);
        break;
        
      default:
        // Number card: normal turn progression
        const nextAfterNumber = room.players[nextPlayerIndex];
        console.log(`${player.name} played ${card.color} ${card.value}. ${nextAfterNumber.name} plays next (4-player normal rule)`);
    }
  ...
  getNextPlayerIndexFrom(room, fromPlayerIndex) {
    return (fromPlayerIndex + room.direction + 4) % 4;
  }
  ...
  getNextPlayerIndex(room) {
    return (room.currentPlayerIndex + room.direction + 4) % 4;
  }
  ...
  getGameState(room) {
    return {
      ...
      fourPlayerRules: true
    };
  }

  /**
   * Get 4-player rule description
   */
  getPlayerCountRules() {
    return {
      playerCount: 4,
      description: '4-Player UNO Rules (Standard)',
      rules: {
        skip: 'Next player loses turn, player after them plays',
        reverse: 'Direction changes only, next player in new direction plays',
      ...
      specialBehaviors: [
        ...
        'Opposite players are 2 positions away',
        'Classic UNO experience with balanced gameplay'
      ]
    };
  }
}

module.exports = UnoGame4Player;