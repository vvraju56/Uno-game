/**
 * POWER CARD RULES - 6 PLAYERS (AUTHORITATIVE)
 * 
...
class PowerCardRules6Player {
  ...
  applySkipCard(gameState) {
    ...
    const nextPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 6) % 6;
    const playerAfterNextIndex = (nextPlayerIndex + newGameState.direction + 6) % 6;
    ...
    console.log(`${skippedPlayer.name} skipped. ${nextPlayer.name} plays next (6-player rule)`);
  ...
  applyReverseCard(gameState) {
    ...
    console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${newGameState.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (6-player rule)`);
  ...
  applyDrawTwoCard(gameState, drawCardCallback) {
    const newGameState = { ...gameState };
    const targetPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 6) % 6;
    const playerAfterTargetIndex = (targetPlayerIndex + newGameState.direction + 6) % 6;
    ...
    console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${nextPlayer.name} plays next (6-player rule)`);
  ...
  applyWildDrawFourCard(gameState, chosenColor, drawCardCallback) {
    ...
    console.log(`${targetPlayer.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${nextPlayer.name} plays next (6-player rule)`);
  ...
  getNextPlayerIndex(gameState, cardType) {
    ...
    switch (cardType) {
      case 'skip':
        const nextPlayerIndex = (currentPlayerIndex + direction + 6) % 6;
        return (nextPlayerIndex + direction + 6) % 6;
        
      case 'reverse':
        return (currentPlayerIndex + direction + 6) % 6;
        
      case 'draw2':
      case 'wild4':
        const targetIndex = (currentPlayerIndex + direction + 6) % 6;
        return (targetIndex + direction + 6) % 6;
  ...
  getPlayerPosition(playerIndex, currentPlayerIndex, direction) {
    const relativeIndex = (playerIndex - currentPlayerIndex + 6 + 6 * 6) % 6;
...
module.exports = PowerCardRules6Player;