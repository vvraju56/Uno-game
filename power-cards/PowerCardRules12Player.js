/**
 * POWER CARD RULES - 12 PLAYERS (AUTHORITATIVE)
 * 
...
class PowerCardRules12Player {
  ...
  applySkipCard(gameState) {
    ...
    const nextPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 12) % 12;
    const playerAfterNextIndex = (nextPlayerIndex + newGameState.direction + 12) % 12;
    ...
    console.log(`${skippedPlayer.name} skipped. ${nextPlayer.name} plays next (12-player rule)`);
  ...
  applyReverseCard(gameState) {
    ...
    console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${newGameState.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (12-player rule)`);
  ...
  applyDrawTwoCard(gameState, drawCardCallback) {
    const newGameState = { ...gameState };
    const targetPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 12) % 12;
    const playerAfterTargetIndex = (targetPlayerIndex + newGameState.direction + 12) % 12;
    ...
    console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${nextPlayer.name} plays next (12-player rule)`);
  ...
  applyWildDrawFourCard(gameState, chosenColor, drawCardCallback) {
    ...
    console.log(`${targetPlayer.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${nextPlayer.name} plays next (12-player rule)`);
  ...
  getNextPlayerIndex(gameState, cardType) {
    ...
    switch (cardType) {
      case 'skip':
        const nextPlayerIndex = (currentPlayerIndex + direction + 12) % 12;
        return (nextPlayerIndex + direction + 12) % 12;
        
      case 'reverse':
        return (currentPlayerIndex + direction + 12) % 12;
        
      case 'draw2':
      case 'wild4':
        const targetIndex = (currentPlayerIndex + direction + 12) % 12;
        return (targetIndex + direction + 12) % 12;
  ...
  getPlayerPosition(playerIndex, currentPlayerIndex, direction) {
    const relativeIndex = (playerIndex - currentPlayerIndex + 12 + 12 * 12) % 12;
...
module.exports = PowerCardRules12Player;