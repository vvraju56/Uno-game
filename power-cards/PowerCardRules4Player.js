/**
 * POWER CARD RULES - 4 PLAYERS (AUTHORITATIVE)
 * 
...
class PowerCardRules4Player {
  ...
  /**
   * Apply Skip card effect in 4-player game
   * @param {Object} gameState - Current game state
   * @returns {Object} New game state
   */
  applySkipCard(gameState) {
    const newGameState = { ...gameState };
    const nextPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 4) % 4;
    const playerAfterNextIndex = (nextPlayerIndex + newGameState.direction + 4) % 4;
    ...
    console.log(`${skippedPlayer.name} skipped. ${nextPlayer.name} plays next (4-player rule)`);
    
    return newGameState;
  }

  /**
   * Apply Reverse card effect in 4-player game
   * @param {Object} gameState - Current game state
   * @returns {Object} New game state
   */
  applyReverseCard(gameState) {
    ...
    console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${newGameState.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (4-player rule)`);
    
    return newGameState;
  }

  /**
   * Apply Draw Two card effect in 4-player game
   * @param {Object} gameState - Current game state
  ...
  applyDrawTwoCard(gameState, drawCardCallback) {
    const newGameState = { ...gameState };
    const targetPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 4) % 4;
    const playerAfterTargetIndex = (targetPlayerIndex + newGameState.direction + 4) % 4;
    ...
    console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${nextPlayer.name} plays next (4-player rule)`);
    
    return newGameState;
  }

  /**
   * Apply Wild Draw Four card effect in 4-player game
   * @param {Object} gameState - Current game state
  ...
  applyWildDrawFourCard(gameState, chosenColor, drawCardCallback) {
    ...
    console.log(`${targetPlayer.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${nextPlayer.name} plays next (4-player rule)`);
  ...
  getNextPlayerIndex(gameState, cardType) {
    ...
    switch (cardType) {
      case 'skip':
        ...
        const nextPlayerIndex = (currentPlayerIndex + direction + 4) % 4;
        return (nextPlayerIndex + direction + 4) % 4;
        
      case 'reverse':
        // Direction changed, move to next in new direction
        return (currentPlayerIndex + direction + 4) % 4;
        
      case 'draw2':
      case 'wild4':
        // Target loses turn, move to player after target
        const targetIndex = (currentPlayerIndex + direction + 4) % 4;
        return (targetIndex + direction + 4) % 4;
  ...
  /**
   * Validate power card play for 4-player game
  ...
  /**
   * Get power card description for 4-player game
   * @param {string} cardType - Type of power card
  ...
  /**
   * Get player position in 4-player game
  ...
  getPlayerPosition(playerIndex, currentPlayerIndex, direction) {
    const relativeIndex = (playerIndex - currentPlayerIndex + 4 + 4 * 4) % 4; // Normalize to 0-3
    
    switch (relativeIndex) {
      case 0: return 'current';
      case 1: return direction > 0 ? 'next' : 'previous';
      case 2: return 'opposite';
      case 3: return direction > 0 ? 'previous' : 'next';
      default: return 'unknown';
    }
  }

  /**
   * Get turn order for 4-player game
   * @param {number} currentPlayerIndex - Current player index
   * @param {number} direction - Direction of play
   * @returns {Array} Array of player indices in turn order
   */
  getTurnOrder(currentPlayerIndex, direction) {
    const turnOrder = [];
    for (let i = 0; i < 4; i++) {
      const playerIndex = (currentPlayerIndex + (i * direction) + 4 * 4) % 4;
...
module.exports = PowerCardRules4Player;