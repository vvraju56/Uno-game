/**
 * POWER CARD RULES - 8 PLAYERS (AUTHORITATIVE)
 * 
...
class PowerCardRules8Player {
  ...
  applySkipCard(gameState) {
    ...
    const nextPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 8) % 8;
    const playerAfterNextIndex = (nextPlayerIndex + newGameState.direction + 8) % 8;
    ...
    console.log(`${skippedPlayer.name} skipped. ${nextPlayer.name} plays next (8-player rule)`);
  ...
  applyReverseCard(gameState) {
    ...
    console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${newGameState.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (8-player rule)`);
  ...
  applyDrawTwoCard(gameState, drawCardCallback) {
    const newGameState = { ...gameState };
    const targetPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 8) % 8;
    const playerAfterTargetIndex = (targetPlayerIndex + newGameState.direction + 8) % 8;
    ...
    console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${nextPlayer.name} plays next (8-player rule)`);
  ...
  applyWildDrawFourCard(gameState, chosenColor, drawCardCallback) {
    ...
    console.log(`${targetPlayer.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${nextPlayer.name} plays next (8-player rule)`);
  ...
  getNextPlayerIndex(gameState, cardType) {
    ...
    switch (cardType) {
      case 'skip':
        const nextPlayerIndex = (currentPlayerIndex + direction + 8) % 8;
        return (nextPlayerIndex + direction + 8) % 8;
        
      case 'reverse':
        return (currentPlayerIndex + direction + 8) % 8;
        
      case 'draw2':
      case 'wild4':
        const targetIndex = (currentPlayerIndex + direction + 8) % 8;
        return (targetIndex + direction + 8) % 8;
  ...
  getPlayerPosition(playerIndex, currentPlayerIndex, direction) {
    const relativeIndex = (playerIndex - currentPlayerIndex + 8 + 8 * 8) % 8;
...
module.exports = PowerCardRules8Player;