/**
 * POWER CARD RULES - 3 PLAYERS (AUTHORITATIVE)
 * 
 * Special considerations for 3-player games:
 * - Reverse only changes direction, doesn't skip anyone
 * - Skip skips only the next player in turn order
 * - Draw cards affect specific next player
 */

class PowerCardRules3Player {
  constructor() {
    this.playerCount = 3;
    this.rules = {
      skip: {
  ...
  /**
   * Apply Skip card effect in 3-player game
   * @param {Object} gameState - Current game state
   * @returns {Object} New game state
   */
  applySkipCard(gameState) {
    const newGameState = { ...gameState };
    const nextPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 3) % 3;
    const playerAfterNextIndex = (nextPlayerIndex + newGameState.direction + 3) % 3;
    ...
    console.log(`${skippedPlayer.name} skipped. ${nextPlayer.name} plays next (3-player rule)`);
    
    return newGameState;
  }

  /**
   * Apply Reverse card effect in 3-player game
   * @param {Object} gameState - Current game state
   * @returns {Object} New game state
   */
  applyReverseCard(gameState) {
    ...
    console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${newGameState.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (3-player rule)`);
    
    return newGameState;
  }

  /**
   * Apply Draw Two card effect in 3-player game
   * @param {Object} gameState - Current game state
  ...
  applyDrawTwoCard(gameState, drawCardCallback) {
    const newGameState = { ...gameState };
    const targetPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 3) % 3;
    const playerAfterTargetIndex = (targetPlayerIndex + newGameState.direction + 3) % 3;
    ...
    console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${nextPlayer.name} plays next (3-player rule)`);
    
    return newGameState;
  }

  /**
   * Apply Wild Draw Four card effect in 3-player game
   * @param {Object} gameState - Current game state
  ...
  applyWildDrawFourCard(gameState, chosenColor, drawCardCallback) {
    ...
    console.log(`${targetPlayer.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${nextPlayer.name} plays next (3-player rule)`);
  ...
  getNextPlayerIndex(gameState, cardType) {
    ...
    switch (cardType) {
      case 'skip':
        ...
        const nextPlayerIndex = (currentPlayerIndex + direction + 3) % 3;
        return (nextPlayerIndex + direction + 3) % 3;
        
      case 'reverse':
        // Direction changed, move to next in new direction
        return (currentPlayerIndex + direction + 3) % 3;
        
      case 'draw2':
      case 'wild4':
        // Target loses turn, move to player after target
        const targetIndex = (currentPlayerIndex + direction + 3) % 3;
        return (targetIndex + direction + 3) % 3;
  ...
  /**
   * Validate power card play for 3-player game
  ...
  /**
   * Get power card description for 3-player game
   * @param {string} cardType - Type of power card
  ...
  /**
   * Get player position in 3-player game
  ...
  getPlayerPosition(playerIndex, currentPlayerIndex, direction) {
    const relativeIndex = (playerIndex - currentPlayerIndex + 3 + 3 * 3) % 3; // Normalize to 0-2
    
    switch (relativeIndex) {
      case 0: return 'current';
      case 1: return direction > 0 ? 'next' : 'previous';
      case 2: return direction > 0 ? 'previous' : 'next';
      default: return 'unknown';
    }
  }
}

module.exports = PowerCardRules3Player;