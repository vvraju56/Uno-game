/**
 * POWER CARD RULES - 10 PLAYERS (AUTHORITATIVE)
 * 
...
class PowerCardRules10Player {
  ...
  applySkipCard(gameState) {
    ...
    const nextPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 10) % 10;
    const playerAfterNextIndex = (nextPlayerIndex + newGameState.direction + 10) % 10;
    ...
    console.log(`${skippedPlayer.name} skipped. ${nextPlayer.name} plays next (10-player rule)`);
  ...
  applyReverseCard(gameState) {
    ...
    console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${newGameState.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (10-player rule)`);
  ...
  applyDrawTwoCard(gameState, drawCardCallback) {
    const newGameState = { ...gameState };
    const targetPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 10) % 10;
    const playerAfterTargetIndex = (targetPlayerIndex + newGameState.direction + 10) % 10;
    ...
    console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${nextPlayer.name} plays next (10-player rule)`);
  ...
  applyWildDrawFourCard(gameState, chosenColor, drawCardCallback) {
    ...
    console.log(`${targetPlayer.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${nextPlayer.name} plays next (10-player rule)`);
  ...
  getNextPlayerIndex(gameState, cardType) {
    ...
    switch (cardType) {
      case 'skip':
        const nextPlayerIndex = (currentPlayerIndex + direction + 10) % 10;
        return (nextPlayerIndex + direction + 10) % 10;
        
      case 'reverse':
        return (currentPlayerIndex + direction + 10) % 10;
        
      case 'draw2':
      case 'wild4':
        const targetIndex = (currentPlayerIndex + direction + 10) % 10;
        return (targetIndex + direction + 10) % 10;
  ...
  getPlayerPosition(playerIndex, currentPlayerIndex, direction) {
    const relativeIndex = (playerIndex - currentPlayerIndex + 10 + 10 * 10) % 10;
...
module.exports = PowerCardRules10Player;