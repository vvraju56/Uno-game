/**
 * POWER CARD RULES - 11 PLAYERS (AUTHORITATIVE)
 * 
...
class PowerCardRules11Player {
  ...
  applySkipCard(gameState) {
    ...
    const nextPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 11) % 11;
    const playerAfterNextIndex = (nextPlayerIndex + newGameState.direction + 11) % 11;
    ...
    console.log(`${skippedPlayer.name} skipped. ${nextPlayer.name} plays next (11-player rule)`);
  ...
  applyReverseCard(gameState) {
    ...
    console.log(`Direction reversed from ${originalDirection > 0 ? 'clockwise' : 'counter-clockwise'} to ${newGameState.direction > 0 ? 'clockwise' : 'counter-clockwise'}. ${nextPlayer.name} plays next (11-player rule)`);
  ...
  applyDrawTwoCard(gameState, drawCardCallback) {
    const newGameState = { ...gameState };
    const targetPlayerIndex = (newGameState.currentPlayerIndex + newGameState.direction + 11) % 11;
    const playerAfterTargetIndex = (targetPlayerIndex + newGameState.direction + 11) % 11;
    ...
    console.log(`${targetPlayer.name} draws 2 cards and loses turn. ${nextPlayer.name} plays next (11-player rule)`);
  ...
  applyWildDrawFourCard(gameState, chosenColor, drawCardCallback) {
    ...
    console.log(`${targetPlayer.name} draws 4 cards and loses turn. Color set to ${chosenColor}. ${nextPlayer.name} plays next (11-player rule)`);
  ...
  getNextPlayerIndex(gameState, cardType) {
    ...
    switch (cardType) {
      case 'skip':
        const nextPlayerIndex = (currentPlayerIndex + direction + 11) % 11;
        return (nextPlayerIndex + direction + 11) % 11;
        
      case 'reverse':
        return (currentPlayerIndex + direction + 11) % 11;
        
      case 'draw2':
      case 'wild4':
        const targetIndex = (currentPlayerIndex + direction + 11) % 11;
        return (targetIndex + direction + 11) % 11;
  ...
  getPlayerPosition(playerIndex, currentPlayerIndex, direction) {
    const relativeIndex = (playerIndex - currentPlayerIndex + 11 + 11 * 11) % 11;
...
module.exports = PowerCardRules11Player;